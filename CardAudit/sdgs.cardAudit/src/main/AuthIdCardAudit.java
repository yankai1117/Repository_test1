...
public class AuthIdCardAudit {
	...
	public String execute(HiATLParam args, final HiMessageContext ctx)
		...
		String realname = HiArgUtils.getStringNotNull(args, "realname");// 车牌号
		logger.info("realname", realname);
	
		String certificatenumber = HiArgUtils.getStringNotNull(args, "certificatenumber");//车辆识别号
		logger.info("certificatenumber", certificatenumber);
		
		String ordernumber = HiArgUtils.getStringNotNull(args, "ordernumber");//	发动机号
 ...
 org.apache.http.client.methods.HttpPost post2 = new HttpPost(tokenURL);
        ...
        try {  
            ...
            if (code==200) {  
		         ...
		         etf.setChildValue("realname", JSONObject.fromObject(obj.get("data")).get("realname").toString());
		         etf.setChildValue("certificatenumber", JSONObject.fromObject(obj.get("data")).get("certificatenumber").toString());
